...
int main(int argc, char* argv[]){
	...
	// Ligne d'input au client (interface)
	char* ligne = NULL;

	while(1){
		printf("Veuillez rentrez une commande : \n");
		if(getline(&ligne, NULL, stdin) == -1){
			printf("Echec...");
			break;
		}
		printf("Commande: %s\n", ligne);
		if(strcmp(ligne, "STOP")){
			strcpy(buffer, "STOP");
			printf("Déconnexion avec le proxy... \n");
